...
public class PlayerService implements PlayerRepository {
    ...
    @Override
    public Player updatePlayer(int playerId, Player player)throws ResponseStatusException{
    ...
    @Override
    public void deletePlayer(int playerId)throws ResponseStatusException{
        Player player = team.get(playerId);
        if(player == null){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
